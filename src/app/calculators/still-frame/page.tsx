...
export default function StillFramePage() {
  // Basic parameters
  ...
  const [dpi, setDpi] = useState<keyof typeof DPI_MULTIPLIERS>('300');
  ...
  const calculatePrice = () => {
    ...
    setTimeout(() => {
      const calculatedPrice = calculateStillFramePrice(
        resolution,
        aspectRatio,
        dpi,
        complexity,
        dpi === 'Custom' ? customDpi : undefined
      );
      ...
      const priceBreakdown = getPriceBreakdown('stillFrame', finalPrice, {
        resolution,
        aspectRatio,
        dpi,
        complexity,
  ...
  return (
    <div className="flex flex-col min-h-screen dark:text-white">
      ...
      <main className="flex-grow container mx-auto px-4 py-8 animate-fade-in">
        ...
        >
          <h1 className="text-3xl md:text-4xl font-bold bg-gradient-to-r from-indigo-600 to-purple-600 dark:from-indigo-400 dark:to-purple-400 text-transparent bg-clip-text mb-2">Still Frame Artwork</h1>
          <p className="text-lg text-gray-600 dark:text-gray-300 mb-6">Calculate accurate pricing for your 3D still frame projects</p>
          ...
          <div className="bg-gradient-to-br from-white to-blue-50 dark:from-gray-800 dark:to-gray-900 rounded-xl shadow-lg p-6 backdrop-blur-sm border border-gray-100 dark:border-gray-700">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <motion.div whileHover={{ scale: 1.02 }} transition={{ type: 'spring', stiffness: 400, damping: 17 }}>
                ...
                <select
                  value={dpi}
                  onChange={(e) => setDpi(e.target.value as keyof typeof DPI_MULTIPLIERS)}
                  className="w-full p-2 bg-white dark:bg-gray-800 border border-gray-300 dark:border-gray-600 rounded-md focus:ring-indigo-500 focus:border-indigo-500 transition-colors"
                >
                  {Object.keys(DPI_MULTIPLIERS).map((dpiOption) => (
